...
export async function start(serialNumber) {
    ...
    try {
        ...
        const rx = /VERSION\s*([^\s]+)\s*CAL:\s*(\d+)\s*(?:R1:\s*([\d.]+)\s*R2:\s*([\d.]+)\s*R3:\s*([\d.]+))?\s*Board ID\s*([0-9A-F]+)\s*(?:USER SET\s*R1:\s*([\d.]+)\s*R2:\s*([\d.]+)\s*R3:\s*([\d.]+))?\s*Refs\s*VDD:\s*(\d+)\s*HI:\s*(\d.+)\s*LO:\s*(\d.+)/;
...
export async function read() {
    if (!isRttOpen) return;
    try {
        const { rawbytes } = await readRTT(100);
        if (rawbytes) {
