...
function handleTriggerDataSet(data) {
    ...
    for (let i = 0; i < data.length; i += 2) {
        ...
        let sample = 0.0;
        console.log('adcvalue: ', adcValue);
        currentMeasurementRange = (adcValue & MEAS_RANGE_MSK) >> MEAS_RANGE_POS;

        const adcResult = (adcValue & MEAS_ADC_MSK);

        sample = getAdcResult(adcResult, currentMeasurementRange);
        console.log('sample: ', String(sample * 1e6));
        ...
        if (serialState === MODE_IDLE) {
            ...
            */
            if (byte === ESC) {
                ...
                } else if ((dataPayload.length > 4) && (dataPayload.length % 2 === 0)) {
