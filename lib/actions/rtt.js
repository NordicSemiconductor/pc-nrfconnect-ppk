...
export async function start() {
    ...
    const stateData = await getHardwareStates();
    ...
    const version = stateData.split('VERSION ')[1].slice(0, 5);
    const calibrationStatus = stateData.split('CAL:')[1].slice(1, 2);
    const boardID = stateData.split('Board ID ')[1].slice(0, 7);

    const voltageRefs = stateData.split('Refs ')[1];
    const vdd = parseInt(voltageRefs.split('VDD: ')[1].slice(0, 5), 10);
...
export async function read() {
    const readCallback = (err, string, rawbytes /* , time */) => {
        ...
        setTimeout(RTT.read(0, 100, readCallback), 10);
    };
